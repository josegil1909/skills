...
|----------|-----------|------------|---------------------|
| Go service | 150 LOC | 200 LOC | Extract subservices |
| HTTP handler | 30 LOC | 50 LOC | Move logic to use case |
| Go repository | 100 LOC | 150 LOC | Extract helper queries |
| Vue SFC | 200 LOC | 300 LOC | Extract sub-components |
| Composable | 100 LOC | 150 LOC | Split by responsibility |
| Pinia store | 100 LOC | 150 LOC | Split into focused stores |
| Go test file | 200 LOC | 300 LOC | Split by scenario |

### AFY Anti-Patterns

| Smell | Example | Solution |
|-------|---------|----------|
| **Tenant context leak** | Passing `tenantID` as bare string everywhere | Use `TenantContext` struct |
| **Repository soup** | Service with 5+ repository dependencies | Max 3 repos; use orchestrators |
| **Store everything** | Pinia store handling cart + checkout + tickets + customer | One store per domain |
| **Handler does everything** | HTTP handler with validation, business logic, DB calls | Handler → use case → response |
| **Type switches** | `if doc.DocType == "..."` in service | Strategy pattern |
| **Currency switches** | `switch currency { case "VES": ... }` | CurrencyConverter registry |
| **God routes file** | `routes.go` with 9000+ LOC | Split by domain |
