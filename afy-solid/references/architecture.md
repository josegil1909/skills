...
```
cmd/               → entry points (server-local, server-cloud)
internal/adapters/ → HTTP, DB, devices, notifications, workers
internal/modules/  → use cases / services
internal/core/     → domain entities + ports (interfaces)
...
**Allowed:**
```go
// Modules depend on ports (abstractions)
package sales
import "internal/core/ports"

// Adapters depend on modules
package http_fiber
import "internal/modules/sales"
```

```typescript
// API layer must not depend on stores
// src/lib/api/http.ts
import { usePosStore } from '@/stores/usePosStore'  // PROHIBITED

// Components can depend on stores and API
